...
<body>
    ...
    <tr>
      ...
      <td><?php 
                if($consommation>=$tarifs[1]->borneMin && $consommation <= $tarifs[1]->borneMax){
                    ...
                    echo ($redevance[$calibre] * $tva /100)+($AYOUB * $tva /100);   
                }
    ...
    <tr>
      ...
      <td><?php
      if($consommation>=$tarifs[1]->borneMin && $consommation <= $tarifs[1]->borneMax){
            echo $AYOUB2 + $redevance[$calibre];
      }
      else{
            echo $AYOUB + $redevance[$calibre];
        }
       ?></td>
      <td></td>
      <td><?php 
                if($consommation>=$tarifs[1]->borneMin && $consommation <= $tarifs[1]->borneMax){
                    echo $totaltranche2+($redevance[$calibre] * $tva /100)+$timbre;
                     ...
                     echo ($redevance[$calibre] * $tva /100)+($AYOUB * $tva /100)+$timbre;
                }
                ?>
                </td>
    ...
    <tr>
      ...
      <td><?php 
       if($consommation>=$tarifs[1]->borneMin && $consommation <= $tarifs[1]->borneMax){
        echo ($totaltranche2+($redevance[$calibre] * $tva /100)+$timbre)+($AYOUB2 + $redevance[$calibre]);
    }
    else{
         echo (($redevance[$calibre] * $tva /100)+($AYOUB * $tva /100)+$timbre)+($AYOUB + $redevance[$calibre]);
    }
        ?>
        </td>
      <td></td>
